...
class ConnectPayload(Packet):
    def __init__(self, header):
        ...
        if threading.current_thread().ident == g.Client_Fuzzing_Thread_ID and g.cur_client_dependency != None:
            if len(g.cur_client_dependency) == 2 and g.FIELD_WILL_TOPIC in g.cur_client_dependency["active"]["field"] and self.will_topic_length > 0:
                g.cur_client_dependency["passive"]["field"][g.FIELD_WILL_TOPIC] = self.will_topic
